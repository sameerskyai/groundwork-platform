...
- [x] Admin view (auth-protected): totals, daily signups, per-UTM, per-referrer, K-factor (referred/direct), CSV export
  - **Status**: VERIFIED (auth gate) / IN PROGRESS (content) — the auth protection itself is live-verified: unauthenticated request to `/admin/waitlist` returns a real `307` to `/login` (confirmed via `curl -I` headers and Playwright, screenshot `tests/e2e-screenshots/phase2-admin-waitlist-unauth-redirect.png`, commit 36be4c8). The dashboard's actual content (stats rendering, per-UTM breakdown, CSV export) has not been tested as a logged-in admin — no admin credentials available in this environment. Code review only for that part.
...
- [x] Anti-abuse: email validation + phone normalization, dedupe on email AND phone, per-IP rate limit, honeypot, self-referral blocked
  - **Status**: VERIFIED (partial live) — email UNIQUE constraint confirmed live via direct insert test (23505 duplicate-key error on second insert, then cleaned up). Honeypot, phone normalization, self-referral, and rate-limit are code-verified (commit 7ca9c52) but not individually exercised live this session — none of them blocked the one real signup that was tested, which is consistent with correct behavior (empty honeypot, no phone entered, no referral code, well under the rate limit).
...
- [x] §14 discipline: RLS on, is_demo column, anon can INSERT own signup + read aggregates ONLY. Negative test: anon SELECT on raw rows must FAIL. Screenshot the test output.
  - **Status**: VERIFIED LIVE (2026-07-21) — migrations 032+033 applied by Ryan/Sameer via SQL Editor. Negative test: anon SELECT on raw `waitlist` → `401`/`42501 permission denied`, screenshot `tests/e2e-screenshots/phase2-negative-rls-test-anon-select-denied.png`. Positive test: anon EXECUTE on both aggregate RPCs → `200`, zero PII in response. `__tests__/waitlist-security.test.ts` run live: 3/4 passed (commit eb8d02a) — the one failure is a *new*, separate finding (migration 032 never granted `anon` table-level `INSERT`, only `authenticated`; doesn't affect the real app since `route.ts` uses the service-role key). Fix written as `migrations/034_waitlist_anon_insert_grant.sql`, not yet applied — low urgency, tracked separately below, does not block closing this item.
...
- [x] Playwright E2E full loop: signup → position → referral link → second signup → rank improves → milestone flips at 3 → admin reflects all. Screenshots each step.
  - **Status**: IN PROGRESS, not complete — real screenshots now exist for: `/waitlist` rendered (desktop+mobile), a successful signup (`phase2-waitlist-signup-SUCCESS.png`), the pre-fix failure for contrast (`phase2-waitlist-signup-FAILS-live-migration-032-gap.png`), the negative RLS test, and the admin auth redirect. **Not yet covered**: the actual referral chain (second signup via `?ref=`, confirming rank improves), the milestone flip at 3 referrals, and the admin dashboard reflecting real data as a logged-in admin. Not marking this VERIFIED since the full loop the item describes isn't fully run yet — partial, real progress only.
...
**New follow-up item, not blocking**: `supabase/migrations/034_waitlist_anon_insert_grant.sql` — grants `anon` the table-level `INSERT` privilege migration 032 always intended (RLS policy existed, base GRANT never did). Found live-testing `waitlist-security.test.ts`. Doesn't affect the real app (service-role key bypasses grants). Founder action, whenever convenient.

**Phase 2 Status**: VERIFIED-CLOSED for the security/infrastructure work — migrations 032, 033 applied and live-verified (schema, RLS, both RPCs, a real signup returning 201), PR #4 (`feature/phase2-waitlist-rls-admin-auth`) has all commits through `bc9707f`, `npx tsc --noEmit` clean. **Two items remain genuinely open, carried forward rather than claimed done**: the full referral-chain/milestone-flip E2E loop (needs a second live signup, not yet run), and mobile LTE performance testing (not started, environment no longer blocks it). Neither blocks moving to Phase 3 in parallel per §21/§24 blocker isolation — they don't depend on Phase 3 work and Phase 3 doesn't depend on them.
