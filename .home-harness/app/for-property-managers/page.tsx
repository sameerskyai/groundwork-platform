    ...
    <div style={{ background: 'var(--color-base-alt)', minHeight: '100vh' }}>
      ...
      <section style={{ background: 'var(--color-ink)', padding: '90px 24px' }}>
        <div style={{ maxWidth: 820, margin: '0 auto' }}>
          ...
          <h1 style={{ color: 'var(--color-base)', fontSize: 'clamp(2.2rem, 5vw, 3.5rem)', fontWeight: 800, lineHeight: 1.1, letterSpacing: '-0.03em', marginBottom: 24 }}>
            One platform for your whole portfolio.
          </h1>
          <p style={{ color: 'var(--color-line)', fontSize: 17, lineHeight: 1.7, marginBottom: 40, maxWidth: 540 }}>
          ...
          <Link href="/contact">
            <button style={{ padding: '15px 28px', borderRadius: 10, background: 'var(--color-accent)', color: 'var(--color-base)', border: 'none', fontSize: 16, fontWeight: 700, cursor: 'pointer' }}>
              Talk to our team →
            </button>
          </Link>
        </div>
      ...
      <section style={{ padding: '90px 24px', background: 'var(--color-base-alt)' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          <h2 style={{ color: 'var(--color-ink)', fontSize: 28, fontWeight: 800, letterSpacing: '-0.03em', marginBottom: 48 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 24 }}>
            {[
              ...
              <div key={b.title} style={{ padding: '28px', background: 'var(--color-base)', borderRadius: 16, border: '1px solid var(--color-line)' }}>
                <div style={{ fontSize: 28, marginBottom: 14 }}>{b.icon}</div>
                <h3 style={{ color: 'var(--color-ink)', fontSize: 16, fontWeight: 700, marginBottom: 8 }}>{b.title}</h3>
                <p style={{ color: 'var(--color-ink-2)', fontSize: 14, lineHeight: 1.7 }}>{b.body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: '80px 24px', background: 'var(--color-ink)' }}>
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          <h2 style={{ color: 'var(--color-base)', fontSize: 28, fontWeight: 800, letterSpacing: '-0.03em', marginBottom: 8 }}>
            Portfolio pricing
          </h2>
          <p style={{ color: 'var(--color-line)', fontSize: 15, marginBottom: 40 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 32 }}>
            {[
              ...
              <div key={t.units} style={{ padding: '24px', background: 'color-mix(in srgb, var(--color-base) 4%, transparent)', border: '1px solid color-mix(in srgb, var(--color-base) 8%, transparent)', borderRadius: 14 }}>
                <div style={{ color: 'var(--color-accent-on-dark)', fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 8 }}>{t.label}</div>
                <div style={{ fontFamily: 'monospace', fontSize: 26, fontWeight: 700, color: 'var(--color-base)', marginBottom: 4 }}>{t.price}</div>
                <div style={{ color: 'var(--color-line)', fontSize: 13 }}>{t.units}</div>
              </div>
            ))}
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {['Full access to cost estimation across all units', 'Contractor matching and messaging', 'Portfolio-wide cost tracking and history', 'Verified contractor pool', 'Priority support'].map(f => (
              <div key={f} style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                <Check style={{ width: 14, height: 14, color: 'color-mix(in srgb, var(--color-verified) 70%, var(--color-base))', flexShrink: 0 }} />
                <span style={{ color: 'var(--color-line)', fontSize: 14 }}>{f}</span>
              </div>
            ))}
          </div>
          <div style={{ marginTop: 36 }}>
            <Link href="/contact">
              <button style={{ padding: '14px 28px', borderRadius: 10, background: 'var(--color-accent)', color: 'var(--color-base)', border: 'none', fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>
