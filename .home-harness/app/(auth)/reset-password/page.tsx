    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4" style={{ background: 'var(--color-surface-primary)' }}>
      <div className="w-full max-w-sm">
        <div className="text-center mb-10">
          ...
          <h1 className="text-2xl font-bold mb-1" style={{ color: 'var(--color-ink)' }}>Set a new password</h1>
        ...
        <div className="rounded-2xl p-6 border" style={{ background: 'var(--color-surface-secondary)', borderColor: 'var(--color-border-strong)' }}>
          {linkState === 'checking' && (
            <p className="text-sm" style={{ color: 'var(--color-ink-2)' }}>Verifying your reset link...</p>
          )}
          ...
          {linkState === 'ready' && (
            <form onSubmit={handleUpdate} className="flex flex-col gap-4">
              {[
                ...
                <div key={f.label}>
                  <label className="block text-sm font-medium mb-1.5" style={{ color: 'var(--color-ink-2)' }}>{f.label}</label>
                  <input
                    type="password"
                    value={f.value}
                    ...
                    style={{ background: 'var(--color-surface-primary)', border: '1px solid var(--color-border-strong)', color: 'var(--color-ink)' }}
                    placeholder={f.placeholder}
                  />
                </div>
              ))}
              {error && <p className="text-[var(--color-alert)] text-sm">{error}</p>}
