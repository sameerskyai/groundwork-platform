  ...
  useEffect(() => {
    async function loadMetrics() {
      try {
        ...
        // Never surface the raw exception: it is a developer string, and on
        // this route it is almost always a permissions denial rather than a
        // crash. Say which of the two it is, in plain language.
  ...
  if (error) {
    return (
      <div className="min-h-screen p-6" style={{ backgroundColor: 'var(--color-surface-primary)' }}>
        <EmptyState
          icon={Users}
          title="This dashboard is for admin accounts"
          description="You are signed in, but this account does not have the admin role, so the platform metrics are not readable from it. Sign in with an admin account, or head back to your own dashboard."
        />
      </div>
    )
  }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {!loading && metrics && Object.values(metrics).every(v => v === 0) && (
          <Card variant="default" className="p-12">
            <EmptyState
              icon={TrendingUp}
              title="Every metric is at zero"
              description="Nothing has happened on the platform yet — no users, projects, matches or messages — so these are true zeros rather than a failed query. The first waitlist invite is what starts the numbers moving."
            />
          </Card>
        )}
