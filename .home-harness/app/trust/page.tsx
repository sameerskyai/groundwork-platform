    ...
    <div style={{ background: 'var(--color-base-alt)', minHeight: '100vh' }}>
      ...
      <section style={{ background: 'var(--color-ink)', padding: '80px 24px', textAlign: 'center' }}>
        <div style={{ maxWidth: 700, margin: '0 auto' }}>
          <Shield style={{ width: 48, height: 48, color: 'var(--color-accent-on-dark)', margin: '0 auto 24px' }} />
      ...
      <section style={{ padding: '80px 24px', background: 'var(--color-base-alt)' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <h2 style={{ color: 'var(--color-ink)', fontSize: 28, fontWeight: 800, letterSpacing: '-0.02em', marginBottom: 40 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            {[
              ...
              <div key={step.n} style={{ display: 'grid', gridTemplateColumns: '48px 1fr auto', gap: 20, padding: '24px', background: 'var(--color-base)', borderRadius: 16, border: '1px solid var(--color-line)', alignItems: 'start' }}>
                <div style={{ width: 48, height: 48, borderRadius: '50%', background: step.color, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--color-base)', fontWeight: 800, fontSize: 18, flexShrink: 0 }}>
                  {step.n}
                </div>
                <div>
                  <h3 style={{ color: 'var(--color-ink)', fontSize: 16, fontWeight: 700, marginBottom: 6 }}>{step.title}</h3>
                  <p style={{ color: 'var(--color-ink-2)', fontSize: 14, lineHeight: 1.7 }}>{step.body}</p>
                </div>
                <div style={{ background: `color-mix(in srgb, ${step.color} 12%, var(--color-base))`, border: `1px solid color-mix(in srgb, ${step.color} 30%, var(--color-base))`, borderRadius: 100, padding: '4px 12px', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <section style={{ padding: '80px 24px', background: 'var(--color-ink)' }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
            <Star style={{ width: 28, height: 28, color: 'var(--color-accent-on-dark)' }} />
            <h2 style={{ color: 'var(--color-base)', fontSize: 28, fontWeight: 800, letterSpacing: '-0.02em' }}>
              The trust score
            </h2>
          </div>
          <p style={{ color: 'var(--color-line)', fontSize: 16, lineHeight: 1.8, marginBottom: 32 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 32 }}>
            {[
              ...
              <div key={m.metric} style={{ padding: '20px', background: 'color-mix(in srgb, var(--color-base) 4%, transparent)', border: '1px solid color-mix(in srgb, var(--color-base) 7%, transparent)', borderRadius: 14 }}>
                <h3 style={{ color: 'var(--color-base)', fontSize: 14, fontWeight: 700, marginBottom: 6 }}>{m.metric}</h3>
                <p style={{ color: 'var(--color-line)', fontSize: 12, lineHeight: 1.6, marginBottom: 8 }}>{m.desc}</p>
                <span style={{ background: 'color-mix(in srgb, var(--color-accent-on-dark) 15%, transparent)', color: 'var(--color-accent-on-dark)', fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 100 }}>{m.weight}</span>
              </div>
            ))}
          </div>
          <div style={{ background: 'color-mix(in srgb, var(--color-base) 4%, transparent)', border: '1px solid color-mix(in srgb, var(--color-base) 8%, transparent)', borderRadius: 14, padding: '20px 24px' }}>
            <p style={{ color: 'var(--color-line)', fontSize: 14, lineHeight: 1.8 }}>
              <strong style={{ color: 'var(--color-base)' }}>New contractors</strong> start with a provisional score until they have enough completed jobs to establish a track record. During this period, they&apos;re clearly labeled as &ldquo;new to Laywork&rdquo; and their base verification badges still display.
            </p>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: '80px 24px', background: 'var(--color-base-alt)' }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          <h2 style={{ color: 'var(--color-ink)', fontSize: 28, fontWeight: 800, letterSpacing: '-0.02em', marginBottom: 32 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {[
              ...
              <div key={item.trigger} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, padding: '20px 24px', background: 'var(--color-base)', borderRadius: 14, border: '1px solid var(--color-line)' }}>
                <div style={{ color: 'var(--color-ink-2)', fontSize: 14, lineHeight: 1.6 }}><strong style={{ color: 'var(--color-ink)' }}>If:</strong> {item.trigger}</div>
                <div style={{ color: 'var(--color-ink-2)', fontSize: 14, lineHeight: 1.6 }}><strong style={{ color: 'var(--color-accent)' }}>Then:</strong> {item.action}</div>
              </div>
            ))}
          </div>
        </div>
