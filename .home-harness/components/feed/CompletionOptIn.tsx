...
export function CompletionOptIn({ projectId, zipCode, onDone }: CompletionOptInProps) {
  ...
  if (step === 'done') {
    return (
      <div className="bg-[var(--color-base-alt)] border border-[var(--color-verified)] rounded-2xl p-5 text-center">
        <p className="text-sm font-medium text-[var(--color-verified)]">Thanks — your project has been recorded.</p>
      </div>
    )
  }
  ...
  if (step === 'street') {
    return (
      <div className="bg-[var(--color-base)] border border-[var(--color-line)] rounded-2xl p-5 shadow-sm">
        <h3 className="font-bold text-[var(--color-ink)] mb-1">Add a street name? (optional)</h3>
        ...
        <input
          ...
          className="w-full px-3 py-2.5 rounded-xl border border-[var(--color-line)] text-sm focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)] mb-4"
        />
    ...
    <div className="bg-[var(--color-base)] border border-[var(--color-line)] rounded-2xl p-5 shadow-sm">
      <div className="flex items-start gap-3">
        <div className="w-9 h-9 rounded-xl bg-[var(--color-accent)] flex items-center justify-center flex-shrink-0">
          <MapPin className="w-4 h-4 text-[var(--color-base)]" />
        </div>
        <div className="flex-1">
          <h3 className="font-bold text-[var(--color-ink)] mb-1">Share this project with your neighborhood?</h3>
          <p className="text-sm text-[var(--color-ink-2)] mb-4">
        ...
        <button
          ...
          className="text-[var(--color-line-strong)] hover:text-[var(--color-ink)] transition-colors mt-0.5"
          aria-label="Dismiss"
        >
          <X className="w-4 h-4" />
        </button>
