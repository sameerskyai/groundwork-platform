    ...
    <div style={{ background: 'var(--color-base-alt)', minHeight: '100vh' }}>
      ...
      <main style={{ maxWidth: 1000, margin: '0 auto', padding: '80px 24px' }}>
        <div style={{ textAlign: 'center', marginBottom: 72 }}>
          <h1 style={{ color: 'var(--color-ink)', fontSize: 44, fontWeight: 800, letterSpacing: '-0.03em', marginBottom: 16 }}>
            Honest pricing. No hidden fees.
          </h1>
          <p style={{ color: 'var(--color-text-muted)', fontSize: 18, maxWidth: 520, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 64 }}>
          <h2 style={{ color: 'var(--color-ink)', fontSize: 22, fontWeight: 800, letterSpacing: '-0.02em', marginBottom: 20 }}>
            For homeowners
          </h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 16 }}>
            {[
              ...
              <div key={t.tier} style={{ padding: '28px', borderRadius: 16, background: t.dark ? 'var(--color-ink)' : 'var(--color-base)', border: t.dark ? 'none' : '1px solid var(--color-line)' }}>
                <div style={{ color: t.dark ? 'var(--color-brand-light)' : 'var(--color-accent)', fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 10 }}>{t.tier}</div>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginBottom: 4 }}>
                  <span style={{ fontFamily: 'monospace', fontSize: 32, fontWeight: 700, color: t.dark ? 'var(--color-base)' : 'var(--color-ink)' }}>{t.price}</span>
                </div>
                <div style={{ color: 'var(--color-text-muted)', fontSize: 13, marginBottom: 20 }}>{t.period}</div>
                {t.features.map(f => (
                  <div key={f} style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
                    <Check style={{ width: 14, height: 14, color: 'var(--color-verified)', flexShrink: 0, marginTop: 2 }} />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 16 }}>
            {[
              ...
              <div key={t.tier} style={{ padding: '28px', borderRadius: 16, background: t.dark ? 'var(--color-ink)' : 'var(--color-base)', border: t.dark ? 'none' : '1px solid var(--color-line)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginBottom: 4 }}>
                  <span style={{ fontFamily: 'monospace', fontSize: 32, fontWeight: 700, color: t.dark ? 'var(--color-base)' : 'var(--color-ink)' }}>{t.price}</span>
                  <span style={{ color: 'var(--color-text-muted)', fontSize: 14 }}>{t.period}</span>
                </div>
                <div style={{ color: t.dark ? 'var(--color-brand-light)' : 'var(--color-accent)', fontSize: 13, fontWeight: 600, marginBottom: 20 }}>{t.cap}</div>
                {t.features.map(f => (
                  <div key={f} style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
                    <Check style={{ width: 14, height: 14, color: 'var(--color-verified)', flexShrink: 0, marginTop: 2 }} />
                    <span style={{ color: t.dark ? 'var(--color-line)' : 'var(--color-ink)', fontSize: 14 }}>{f}</span>
        ...
        <div style={{ marginBottom: 48 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
            {[
              ...
              <div key={t.tier} style={{ padding: '24px', background: 'var(--color-base)', borderRadius: 14, border: '1px solid var(--color-line)', textAlign: 'center' }}>
                <div style={{ color: 'var(--color-accent)', fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 10 }}>{t.tier}</div>
                <div style={{ fontFamily: 'monospace', fontSize: 26, fontWeight: 700, color: 'var(--color-ink)', marginBottom: 4 }}>{t.price}</div>
                <div style={{ color: 'var(--color-text-muted)', fontSize: 13 }}>{t.units}</div>
              </div>
            ))}
          </div>
          <div style={{ marginTop: 20 }}>
            <Link href="/contact">
              <button style={{ padding: '13px 24px', borderRadius: 10, background: 'var(--color-accent)', color: 'var(--color-base)', border: 'none', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
        ...
        {/* FAQ */}
        <div style={{ padding: '40px', background: 'var(--color-ink)', borderRadius: 20 }}>
          <h2 style={{ color: 'var(--color-base)', fontSize: 20, fontWeight: 700, marginBottom: 28 }}>Pricing FAQ</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            {[
              ...
              <div key={item.q} style={{ borderBottom: '1px solid color-mix(in srgb, var(--color-base) 7%, transparent)', paddingBottom: 20 }}>
                <h3 style={{ color: 'var(--color-base)', fontSize: 15, fontWeight: 600, marginBottom: 8 }}>{item.q}</h3>
                <p style={{ color: 'var(--color-line)', fontSize: 14, lineHeight: 1.7 }}>{item.a}</p>
              </div>
            ))}
          </div>
        </div>
