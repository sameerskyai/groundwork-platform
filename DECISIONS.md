...
## CodeRabbit review — PR #8 (Gate 4 walkthrough prep), findings addressed (2026-07-22)

6 findings. 5 fixed, 1 acknowledged-but-not-actionable-by-me (logged here per WARP.md §19 rather than silently ignored).

1. **Fixed — SECURITY DEFINER helpers were probe-able (migration 037).** `is_community_member(p_community_id, p_user_id)` took the user id as a caller-supplied argument. SECURITY DEFINER functions in the `public` schema get `EXECUTE` granted to PUBLIC by default and are directly callable as Supabase RPCs, so anyone could have called it with arbitrary `(community, user)` pairs to probe membership. Rewrote to read `auth.uid()` internally (dropping the parameter entirely) and added explicit `REVOKE EXECUTE ... FROM PUBLIC, anon` + `GRANT ... TO authenticated`. This is the same class of bug as the migration 035 → 036 anon-EXECUTE gap from the earlier waitlist work — worth noting the pattern has now recurred twice.
2. **Fixed — same issue in migration 038.** `is_contractor_matched_to_own_project(p_contractor_id, p_user_id)` had the identical caller-supplied-user-id problem; would have let anyone probe which contractors are matched to any given user's projects. Same fix applied.
3. **Fixed — failed count queries rendered as "0 members · 0 posts".** The new count queries in `communities/page.tsx` discarded their errors, so an RLS or query failure silently displayed as legitimate zeros. Now captures both errors and throws before the `?? 0` fallback applies (which now only covers a genuine null count, not a failure).
4. **Fixed — Bug #5 test never exercised the branch it was testing.** The chat back-link test opened `/homeowner/chat` with no `match` param, so `matchProjectId` never loaded and only the `/homeowner` fallback path ran — the project-aware branch (the one that actually had the bug) was never touched, and the result was only `console.log`ed, never asserted. Rewrote to fetch a real seeded match id at runtime, navigate with it, assert the back-link's `href` matches `/homeowner/matches?project=`, click it, and assert the destination renders real matches (waits out the loading spinner first — the initial screenshot caught a spinner, which is weak evidence for a walkthrough packet). Also kept an explicit assertion for the no-match fallback branch.
5. **Fixed — seed regression check couldn't fail.** `console.log('matches page shows 92%:', content.includes('92%'))` passed whether or not the seeded match existed. Changed to `expect(content).toContain('92%')`. Given that this session's *entire* Bug #4 root cause was silently-vanished seed data, a regression check that can't fail was actively harmful.
6. **Acknowledged, not actionable by me — "do not release while these migrations remain manual" (flagged Critical).** CodeRabbit is correct on the substance: Communities stays broken until 037 is applied, and the anonymous `contractor_profiles` demo leak stays live until 038 is applied, and documenting a founder action does not remediate a live environment. I can't comply mechanically — applying either migration requires DDL access this environment doesn't have (no DB password, no Supabase PAT; same constraint as migrations 032/035/036, all of which were founder-applied). What I did instead of quietly proceeding: put both migrations in a **⚠️ READ FIRST** block at the very top of `GATE_WALKTHROUGH_PREP.md` with explicit "what breaks without this" consequences and a copy-pasteable verification query for 038, and listed the recurring bottleneck itself as an open question for Ryan (packet item 4 — whether scoped DDL access can be granted to remove this failure mode). Flagging the honest status: **this PR does not fix the contractor_profiles leak in production. It only writes the fix.** The leak is live until someone applies 038.

**Second CodeRabbit pass on PR #8 (3 further findings, all real, all fixed):**

7. **Removed the credential from my file — but CodeRabbit's severity framing does not survive checking, and I initially repeated it uncritically.** CodeRabbit flagged the demo password in `GATE_WALKTHROUGH_PREP.md` as Major, "treat repository history as compromised." Removed it from my file regardless (a walkthrough packet doesn't need the password inline). But three facts materially change the picture, and I should have checked them before echoing the severity:
   - **This credential was already committed in 6 other files** long before this PR: `DEMO_MODE.md`, `FINAL_REVIEW.md` (×3), `POLISH_SUMMARY.md`, `CORRECTIONS_APPLIED.md`, and `DECISIONS.md` itself. My file added a 7th instance of an already-public string. Removing only mine changes nothing about exposure.
   - **There is a prior explicit decision accepting this** (DECISIONS.md, "Demo Admin Password Hygiene"): plaintext deemed acceptable because the account reaches demo data only, with migration 018 adding RESTRICTIVE UPDATE/DELETE policies as the mitigation. That decision was never revisited.
   - **Verified live against the DB rather than trusting the docs**: `founder.demo@example.com` is `role: "homeowner"`, `is_demo: true`. It is *not* an admin account, despite `FINAL_REVIEW.md` labelling it "Admin User" in three places — that label is stale/wrong and is itself worth correcting, since it makes the credential look far more dangerous than it is.

   So: real hygiene issue, worth rotating, but not the "compromised repository" event the review framing implies, and not something this PR introduced. Did **not** rotate it myself — the same password is in `tests/helpers/auth.ts` and the entire Playwright suite authenticates with it, so a unilateral rotation would have silently broken every test immediately before a walkthrough. Flagged in the packet (Environment note + open item #9) with the full context, including that removing it from *one* file while six others still contain it is not a fix.
8. **Fixed — my own migration 038 fix contradicted itself.** Finding #2 above had me `REVOKE EXECUTE ... FROM anon` on `is_contractor_matched_to_own_project`, but the RESTRICTIVE SELECT policy I wrote in the same pass called that helper for *all* roles including anon. Net effect: an anonymous SELECT touching a demo row would have errored on the missing EXECUTE privilege rather than cleanly filtering the row out — turning a data-leak fix into an availability bug on the public contractor listing. Split into two role-scoped RESTRICTIVE policies: `TO anon` gets the plain `is_demo = false` check, `TO authenticated` gets the helper call. Worth noting this is the second time in this PR that a security hardening step introduced a new bug that only showed up when someone actually traced the privilege interaction.
9. **Fixed — MD040**, fenced code block in the packet had no language specified. `text`.
